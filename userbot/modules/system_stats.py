...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit(
            "**⚜-**⚡️Dezky-Userbot⚡️ Versi:** \n "
            f"heads/DEZKY-USERBOT-0-x634i7u1"
            "\n**⚜-**Revisi:**\n "
            f"{revout}"
        )
    else:
...
async def amireallyalive(alive):
    ...
    output = (
        ...
        f"╰✠╼━━━━━━━━━━━━━━━✠╯\n"
        f"[ɢʀᴏᴜᴘꜱ](https://t.me/leave_to_me) | [ʙᴏᴛᴏꜰ](https://t.me/{user.username}) | [ɢɪᴛʜᴜʙ](https://github.com/KykanoRobot/DEZKY-USERBOT)"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
...
@register(outgoing=True, pattern=r"^\.(?:dezkyon)\s?(.)?")
async def amireallyalive(alive):
    await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
        f"●▬▬▬▬▬▬ஜ۩۞۩ஜ▬▬▬▬▬▬● \n"
        f"✾ 🤴 • `ᴏᴡɴᴇʀ :`[DEZKY](t.me/staytomoonn)             ㅤ \n"
...
CMD_HELP.update(
    {
        "alive": "𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.alive` or `.on` or `.dezky`"
